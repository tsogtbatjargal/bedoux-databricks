...
| 02 | `series/02-quality-gate` | Defend before damage spreads | PR #3 open; deployed and run live in `dev` — found a blocking defect: gate reports pass while the clean/quarantine tables are silently empty (see handoff). Not demonstrated correctly; do not merge | Not drafted |
