...
| 04 | `series/04-investigate-recover` | Investigate and recover | In progress: first increment integrated (guarded model-call path, `model_call.py`, fake provider only, 22 tests; PR #18, merge `b347a1c`, eighth CI deployment — `Resources: 0 created, 0 changed, 0 deleted, 8 unchanged`). Second increment integrated: local incident log (`incidents.py`) and `CheckedPayload` so `send_payload` can't skip the gate; 25 tests; PR #20, merge `27e20c1`, ninth CI deployment — `Resources: 0 created, 0 changed, 0 deleted, 8 unchanged`. Third increment (report citations checked against the sent payload, and a leak screen before a report is returned or stored; 23 tests) on `feat/04-report-citations`, PR open, not merged. Read-only tools not started — see `chapters/04-investigate-recover.md` | Not drafted |
