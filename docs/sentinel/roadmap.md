...
| 03 | `series/03-protect-evidence` | Protect what matters | Integrated into `main` (PR #6, two pre-merge review rounds; PR #8, branch `series/03-evidence-leak-fix`, a post-merge round fixing `evaluate_evidence_gate`'s own problem strings leaking the secret they were reporting), **not acceptance-complete**: redaction/canary spec implemented and unit-tested (`evidence.py`, 149 tests repo-wide) meets "blocked or redacted" and "non-sensitive evidence survives" and documents the inspected boundary, but "a failed check prevents the external call" is structurally blocked until chapter 04 builds a call site — no model call exists yet, and a Delta append is not that call. Durable-evidence-log half built, deployed, and partially demonstrated live (`evidence_log.py`, append-only `gate_evidence_log`, wired into `bedoux_gate_task`; PR #11, merge `7856b78`, sixth CI deployment; runs `649621694823474` and `102698121895581` in `dev` confirmed a row per run matching `gate_status` and `delta.appendOnly` genuinely set — see `chapter-03-evidence.md`; still unobserved live: the property rejecting a mutation, and a row from a failing run; PR #14, merge `028b27e`, seventh CI deployment, a docstring-only fix to `evaluate_evidence_gate` that moved 0 resources — 0 created, 0 changed, 0 deleted, 8 unchanged). See `chapters/03-protect-evidence.md`'s "Roadmap acceptance mapping" | Not drafted |
