...
| 06 | `series/06-agent-boundaries` | Establish rules of command | In progress. First step integrated (PR #26, merge `db3d0e9`, fourteenth CI deployment — `Resources: 0 created, 0 changed, 0 deleted, 8 unchanged`): instructions inside evidence can't dismiss an incident, add a tool, approve or trigger a recovery, or clear a failed gate; a regex flag `instruction_in_evidence`, easy to evade and labelled so. Second step on `feat/06-audit-and-prohibited`, not merged: an audit record of routing decisions and a prohibited-action list, both met at code level. Fake providers only. See `chapters/06-rules-of-command.md` | Not drafted |
