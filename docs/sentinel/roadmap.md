...
| 04 | `series/04-investigate-recover` | Investigate and recover | In progress: first increment integrated (guarded model-call path, `model_call.py`, fake provider only, 22 tests; PR #18, merge `b347a1c`, eighth CI deployment — `Resources: 0 created, 0 changed, 0 deleted, 8 unchanged`). Second increment integrated: local incident log (`incidents.py`) and `CheckedPayload` so `send_payload` can't skip the gate; 25 tests; PR #20, merge `27e20c1`, ninth CI deployment — `Resources: 0 created, 0 changed, 0 deleted, 8 unchanged`. Third increment integrated (report citations checked against the sent payload, and a leak screen inside `send_payload` before a report is returned or stored; 28 tests; PR #21, merge `8f9d1f5`, tenth CI deployment — `Resources: 0 created, 0 changed, 0 deleted, 8 unchanged`). Fourth increment integrated (read-only tools with an allowlist, gated tool results, and a 3-call limit; 25 tests; PR #22, merge `3fa08a9`, eleventh CI deployment — `Resources: 0 created, 0 changed, 0 deleted, 8 unchanged`). Fifth increment (recovery approval with fake executors, duplicate-free replay keyed on `lead_id` offline; 24 tests) on `feat/04-recovery-approval`, PR open, not merged. Acceptance if it merges: five criteria met at code level, offline/live separation holds trivially, the business metric deliberately deferred with the live run; none met live. The live run is deliberately deferred (no real provider). What closes the chapter is the user's decision — see "Roadmap acceptance mapping" in `chapters/04-investigate-recover.md` | Not drafted |
