...
| 06 | `series/06-agent-boundaries` | Establish rules of command | In progress: first step on the chapter branch, not merged (instructions inside evidence are data: text can't dismiss an incident, add a tool, approve or trigger a recovery, or clear a failed gate; a regex flag `instruction_in_evidence`, easy to evade and labelled so). Fake providers only. See `chapters/06-rules-of-command.md` | Not drafted |
