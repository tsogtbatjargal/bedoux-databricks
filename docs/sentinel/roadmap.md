...
| 04 | `series/04-investigate-recover` | Investigate and recover | In progress: first increment integrated (guarded model-call path, `model_call.py`, fake provider only, 22 tests; PR #18, merge `b347a1c`, eighth CI deployment — `Resources: 0 created, 0 changed, 0 deleted, 8 unchanged`). Second increment (local incident log, `incidents.py`, 19 tests) on `feat/04-incident-log`, PR open, not merged. Later increments not started — see `chapters/04-investigate-recover.md` | Not drafted |
