...
| 05 | `series/05-jev-routing` | Spend intelligence carefully | In progress: first step (routing with fake classifier/reasoning models, built on chapter 04's gated path; frozen held-out set; non-dismissable severe signals; 28 tests) on `series/05-jev-routing`, PR open, not merged. No real model called; costs are labelled estimates, latency unmeasured. See `chapters/05-spend-intelligence.md` | Not drafted |
